...
import { type UnitSystem, formatWeight } from '@/util/units';
...
export function RecipeCard({ recipe, showOil, units, onToggleUnits }: RecipeCardProps) {
  ...
  if (showOil) {
    ingredients.push({ name: 'Oil', mass: recipe.oilMass, testId: 'oil-amount' });
  }
    ...
    <div className="rounded-[14px] bg-surface-dark p-[22px] text-[#faf6f1]">
      <div className="mb-4 flex items-center justify-between">
        ...
        <div className="flex gap-1">
          <button type="button" onClick={() => onToggleUnits('metric')}
            className={`rounded px-2 py-0.5 font-sans text-xs font-medium transition-colors ${
              units === 'metric' ? 'bg-primary text-white' : 'text-[#a89580] hover:text-[#faf6f1]'
            }`}>g</button>
          <button type="button" onClick={() => onToggleUnits('imperial')}
            className={`rounded px-2 py-0.5 font-sans text-xs font-medium transition-colors ${
              units === 'imperial' ? 'bg-primary text-white' : 'text-[#a89580] hover:text-[#faf6f1]'
            }`}>oz</button>
        </div>
      </div>
      {ingredients.map((item) => (
        <div key={item.name} className="flex items-baseline justify-between border-b border-white/[0.08] py-2 last:border-b-0">
          <span className="font-sans text-[15px]">{item.name}</span>
          <span className="font-sans text-lg font-semibold">
            <span data-testid={item.testId}>{formatWeight(item.mass, units)}</span>
